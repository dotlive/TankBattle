...
namespace Main
{
    ...
    public class Match : MonoBehaviour
    {
        ...
        {
            var setting = TeamSettings[(int) team];
            var scriptType = Type.GetType(setting.TankScript);
            ...
            var tank = (GameObject) Instantiate(Resources.Load("Tank"), GetRebornPos(team), Quaternion.identity);
            Utils.ChangeTankColor(tank, GetTeamColor(team));

            var t = (Tank)tank.AddComponent(scriptType);
            t.Team = team;
            m_Tanks.Add(t);
        ...
        private void AddStar(bool isSuperStar)
        {
            ...
            if (hasValidPos)
            {
                var starGO = (GameObject) Instantiate(Resources.Load(isSuperStar ? "SuperStar" : "Star"));
                var s = starGO.GetComponent<Star>();
        ...
        {
            var missileGO = (GameObject) Instantiate(Resources.Load("Missile"));
        ...
        void Update()
        {
            ...
            if (m_RemainingTime < 0)
            {
                ...
                if (m_Winner != null)
                {
                    ...
                    if (WinnerShow != null)
                    {
                        Utils.ChangeTankColor(WinnerShow, GetTeamColor(m_Winner.Team));
                    }
                }
        ...
        void OnGUI()
        {
            ...
            if (m_Tanks.Count > 1)
            {
                ...
                GUI.Label(new Rect(Screen.width * 0.5f, 10, Screen.width * 0.5f - 10, 100), m_Tanks[1].GetTankInfo(),
                    m_TeamBInfoStyle);
            }
